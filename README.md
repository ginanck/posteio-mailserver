...
To build and deploy this image to your own container registry, follow these steps:

#### Prerequisites

- Docker installed and running
- Access to your Harbor registry (https://harbor.internal.guru/)
- Appropriate permissions to push to your Harbor project

#### Build and Push Steps

1. **Clone the repository:**
```bash
git clone https://bitbucket.org/ginanck/posteio-mailserver.git
```
...
3. **Build the image with your Harbor registry URL:**
```bash
docker build -t harbor.internal.guru/infra-hub/posteio-mailserver:2.4.10 -t harbor.internal.guru/infra-hub/posteio-mailserver:latest .
```

4. **Login to your Harbor registry:**
```bash
docker login harbor.internal.guru
```

5. **Push the image to Harbor:**
```bash
docker push harbor.internal.guru/infra-hub/posteio-mailserver:2.4.10
```

#### Project Naming

Replace `your-project` with an actual project name in your Harbor instance. Common project names might be:
- `infrastructure` → `harbor.internal.guru/infra-hub/posteio-mailserver:latest`
- `mail` → `harbor.internal.guru/mail/posteio-mailserver:latest`
- `services` → `harbor.internal.guru/services/posteio-mailserver:latest`

**Important:** Make sure the project exists in your Harbor instance before pushing, or create it through the Harbor web UI first.

#### Version Tagging

You can also tag with a specific version instead of `latest`:
```bash
docker build -t harbor.internal.guru/infra-hub/posteio-mailserver:2.4.10 .
docker push harbor.internal.guru/infra-hub/posteio-mailserver:2.4.10
```

### Basic Usage

To use this image, just replace `analogic/poste.io` in your config with your Harbor registry URL. For example, you might use something like this as your `docker-compose.yml`, replacing `mail.example.com` with a suitable hostname for your installation:

```yaml
version: "2.3"
services:
  poste:
    image: harbor.internal.guru/infra-hub/posteio-mailserver:2.4.10
    restart: always
